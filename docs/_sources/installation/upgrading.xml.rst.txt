...
` <https://nixos.org/channels>`_. (Note that the URIs of the
...
` <https://nixos.org/nixos/download.html>`_ to find the newest
...
.. _sec-upgrading-automatic:

Automatic Upgrades
~~~~~~~~~~~~~~~~~~

You can keep a NixOS system up-to-date automatically by adding the following
to :file:`configuration.nix`:

.. parsed-literal::

    :ref:`opt-system.autoUpgrade.enable`  = true;
    :ref:`opt-system.autoUpgrade.allowReboot`  = true;

This enables a periodically executed systemd service named
``nixos-upgrade.service``. If the ``allowReboot``
option is ``false``, it runs :command:`nixos-rebuild switch
--upgrade` to upgrade NixOS to the latest version in the current
channel. (To see when the service runs, see :command:`systemctl list-timers`.)
...
.. parsed-literal::
    ...
    :ref:`opt-system.autoUpgrade.channel`  = https://nixos.org/channels/nixos-19.09;
