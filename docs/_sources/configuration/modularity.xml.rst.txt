...
:file:`configuration.nix` becomes too big, you can split it into
multiple files. Likewise, if you have multiple NixOS configurations (e.g. for
different computers) with some commonality, you can move the common
configuration into a shared file.

Modules have exactly the same syntax as
:file:`configuration.nix`. In fact,
:file:`configuration.nix` is itself a module. You can use other
modules by including them from :file:`configuration.nix`, e.g.:
...
.. parsed-literal::
    ...
    { imports = [ ./vpn.nix ./kde.nix ];
      ...
      :ref:`opt-environment.systemPackages`  = [ pkgs.emacs ];
      *...*
    }

Here, we include two modules from the same directory,
:file:`vpn.nix` and :file:`kde.nix`. The latter
...
Note that both :file:`configuration.nix` and
:file:`kde.nix` define the option
:ref:`opt-environment.systemPackages`. When multiple modules
define an option, NixOS will try to *merge* the
definitions. In the case of :ref:`opt-environment.systemPackages`,
that’s easy: the lists of packages can simply be concatenated. The value in
:file:`configuration.nix` is merged last, so for list-type
options, it will appear at the end of the merged list. If you want it to
appear first, you can use ``mkBefore``:

.. parsed-literal::

    :ref:`opt-boot.kernelModules`  = mkBefore [ "kvm-intel" ];
...
modules define :ref:`opt-services.httpd.adminAddr`,
:command:`nixos-rebuild` will give an error:
::
...
.. parsed-literal::
    ...
    :ref:`opt-services.httpd.adminAddr`  = pkgs.lib.mkForce "bob@example.org";
...
:ref:`opt-environment.systemPackages` only if
:ref:`opt-services.xserver.enable` is set to
``true`` somewhere else:
...
.. parsed-literal::
    ...
    { :ref:`opt-environment.systemPackages`  =
        if config.:ref:`opt-services.xserver.enable`  then
          [ pkgs.firefox
            pkgs.thunderbird
          ]
        else
...
::
    ...
    $ nixos-option :ref:`opt-services.xserver.enable`
    true
