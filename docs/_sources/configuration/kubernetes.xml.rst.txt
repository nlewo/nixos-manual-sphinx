...
.. parsed-literal::
    ...
    :ref:`opt-services.kubernetes.roles`  = [ "master" ];

While this will enable the kubelet and kube-proxy only:

.. parsed-literal::

    :ref:`opt-services.kubernetes.roles`  = [ "node" ];
...
.. parsed-literal::
    ...
    :ref:`opt-services.kubernetes.roles`  = [ "master" "node" ];

Note: Assigning either role will also default both
:ref:`opt-services.kubernetes.flannel.enable` and
:ref:`opt-services.kubernetes.easyCerts` to true. This sets up
...
:ref:`opt-services.kubernetes.apiserver.insecurePort` and
:ref:`opt-services.kubernetes.apiserver.insecureBindAddress`

.. note::

   As of NixOS 19.03, it is mandatory to configure:
   :ref:`opt-services.kubernetes.masterAddress`. The masterAddress
...
also:
`Using RBAC Authorization <https://kubernetes.io/docs/reference/access-authn-authz/rbac/>`_.

The NixOS kubernetes module provides an option for automatic certificate
bootstrapping and configuration,
:ref:`opt-services.kubernetes.easyCerts`. The PKI bootstrapping
...
:ref:`opt-services.kubernetes.easyCerts` = true is sufficient and
...
:ref:`opt-services.kubernetes.easyCerts` is enabled, the helper
...
:ref:`opt-services.kubernetes.pki.etcClusterAdminKubeconfig`.
