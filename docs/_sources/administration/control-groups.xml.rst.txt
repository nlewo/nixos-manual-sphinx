...
The command :command:`systemd-cgls` lists all control groups in the
...
Similarly, :command:`systemd-cgls cpu` shows the cgroups in the CPU
...
service’s CPU share in :file:`configuration.nix`:
...
limits can be specified in :file:`configuration.nix`; for
...
The command :command:`systemd-cgtop` shows a continuously updated
