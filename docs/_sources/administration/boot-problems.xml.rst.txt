...
``boot.debug1``
    ...
    mounted, except for :file:`/proc` and
    :file:`/sys`.
...
``single``
    ...
    runs :command:`sulogin` to prompt for the root password and start
...
:command:`agetty` login prompts should appear eventually unless
