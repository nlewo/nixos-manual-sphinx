...
.. parsed-literal::
    ...
    containers.database =
      { config =
          { config, pkgs, ... }:
          { :ref:`opt-services.postgresql.enable`  = true;
          :ref:`opt-services.postgresql.package`  = pkgs.postgresql_9_6;
          };
      };
...
:file:`configuration.nix` and run ``nixos-rebuild
